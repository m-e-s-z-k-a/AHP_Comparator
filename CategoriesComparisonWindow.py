...
from AlternativesComparisonWindow import AlternativesComparisonWindow

class CategoriesComparisonWindow:
    ...
    def __init__(self, calculations_matrix, alternatives_array):
        ...
        self.root.title("Categories comparison")
        ...
        self.categories_label = tkinter.Label(frame1, text="Compare categories on a scale from 1 to 9",
                                              font=("Gill Sans MT", 18)).pack()
    ...
    def _generate_first_label(self):
        return self.attributes[self.first_index]

    def _generate_second_label(self):
        return self.attributes[self.second_index]

    def _move_to_alternatives_comparison(self):
        self.root.destroy()
        AlternativesComparisonWindow(self.calculations_matrix, self.alternatives_array)
        pass
    ...
    def _submit(self):
        ...
        self.calculations_matrix["criteria_between_eo"][self.first_index][self.second_index] = 1 / val
        self.calculations_matrix["criteria_between_eo"][self.second_index][self.first_index] = val

        self.second_index += 1
        if self.second_index == len(self.attributes):
            self.first_index += 1
            self.second_index = self.first_index + 1

        if self.first_index >= len(self.attributes) - 1:
            self._move_to_alternatives_comparison()
            return
